...
public class TeeladenAnwendungssystem {
	...
	private void nehmeTeeAuf() {
		try {
			this.teesorte = new Teesorte(Integer.parseInt(txtIdentnummer.getText()), txtBezeichnung.getText(),
					txtKategorie.getText(), txtMitKoffein.getText(), txtEnthalteneKraeuter.getText().split(";"));
	...
	private void zeigeTeesorteAn() {
		if (this.teesorte != null) {
			txtAnzeige.setText(this.teesorte.gibTeesorteZurueck(' '));
		} else {
	...
	private void leseAusDatei(String typ) {
		try {
			if ("csv".equals(typ)) {
				...
				this.teesorte = new Teesorte(Integer.parseInt(zeile[0]), zeile[1], zeile[2], zeile[3],
						zeile[4].split("_"));
				ein.close();
				zeigeInformationsfensterAn("Die Teesorte wurde gelesen!");
			} else {
	...
	private void schreibeTeesInCsvDatei() {
		try {
			...
			aus.close();
			zeigeInformationsfensterAn("Die Teesorten wurden gespeichert!");
		} catch (IOException exc) {
